...
class discountDtoWhole:
    ...
    def __init__(self, discount):
        ...
        self.socials = models.SocialNet.objects.filter(
            company=discount.company
        )
...
class discountDtoShort:
    ...
    def __init__(self, discount):
        self.id = discount.id
        self.description = (
            models.Description.objects.filter(discount=discount)
            .get()
            .description
        )
        self.days = discount.description.days
        ...
        self.city = (
            models.Address.objects.filter(company=self.company).get().city
        )
        self.views = (
            models.WatchedAmount.objects.filter(discount=discount).get().amount
        )
        self.percentage = discount.percentage
